...
## ✨ 更新内容
- 新增管理面板 **用量统计**：汇总请求次数、近似上下行流量，并按协议 / 接入节点（Cloudflare colo）/ 出站路径 / 客户端与订阅格式展示卡片与柱状图
- 代理（VLESS / Trojan / XHTTP）与订阅拉取写入 **KV 聚合计数**（键 `usage`），不记录访问目标、不保存原始客户端 IP
- 免费 Workers 友好：隔离体内先聚合，默认至少间隔 90 秒才写入 KV；打开统计页会强制落盘
- 留存最近 **14 个北京时间自然日**；面板可单独清空统计，全量「重置」也会一并清除
...
| 用量统计 | `https://你的域名/<UUID>/api/stats`（需面板鉴权；GET 读取 / POST `?op=reset` 清空） |
...
| 14 | **用量统计** | 面板「用量统计」页：请求/近似流量、按协议、按接入 colo、按出站、按客户端/订阅格式；数据聚合写入 KV `usage` |
...
| **用量统计** | 请求次数、近似流量、24 小时柱状图、按协议 / 接入节点 / 出站 / 客户端拆分 |
...
- **KV 写入节流**：`usage` 与轮询键 `issued` 均默认至少间隔 90 秒才 `put`；`issued` 窗口未变化则跳过写入，避免订阅刷新打满免费套餐每日 1000 次写入

---

## 八、用量统计

部署后在面板顶部打开 **用量统计**。实现全部在 `CFNext 明文版.js`（重新部署该文件即可；混淆版需自行按明文重新生成）。

### 8.1 记什么、不记什么

| 计入 | 不计入 |
|---|---|
| WebSocket 代理（VLESS / Trojan）连接次数与进出 Worker 的近似字节 | 面板页面、登录、配置 API、优选测速 |
| XHTTP 代理连接次数与近似字节 | 访问目标域名 / IP（隐私：Worker 虽能看到出站地址，统计不落盘） |
| 订阅拉取次数与订阅正文大小 | 原始客户端 IP（仅保存 `MD5(盐\|IP\|UA族)` 前 8 位，用于去重与「客户端 · 哈希」） |

协议维度：`vless` / `trojan` / `xhttp` / `订阅`。  
节点维度：Cloudflare **边缘 colo**（如 HKG、LAX）。订阅里的优选 IP 是客户端拨号地址，请求到达 Worker 后看不到具体优选 IP，因此用 colo 作为最接近的「接入节点」。出站另记：`直连` / `自定义落地` / `地区反代-XX` / `UDP/DNS`。  
客户端维度：订阅按 UA 或 `/sub/<格式>` 路径归为 Clash / Sing-box / Surge / Loon / Quantumult X / v2ray 等；代理再附加短哈希以区分不同设备。

流量为隧道两侧经过 Worker 的原始字节（含少量协议头），**不是**运营商账单级带宽，也不是 Cloudflare 控制台的「Workers 请求数」原样复制。

### 8.2 存储、留存与重置

- 使用已绑定的 KV **`K`**，额外键名 **`usage`**（与 `config`、`issued` 并列）。不需要 D1 / R2 / Analytics Engine。
- 按 **北京时间（UTC+8）** 自然日分桶，单键内存最近 **14 天**；更早的日期在写入时丢弃。
- 隔离体内先累加，默认 **至少 90 秒** 才 `put` 一次，避免打满免费 KV 每日写入额度。打开统计页会合并隔离体缓冲展示；仅当缓冲有新数据且距上次写入超过 15 秒才落盘。未绑定 KV 时统计只活在当前隔离体，冷启动后丢失。
- 轮询键 `issued` 同样节流：订阅客户端频繁刷新时不再每次 `put`。
- 统计页「清空统计」只删 `usage`。面板「重置」会清空 `config`、`issued` 与 `usage`。
- 各维度 Top 项有上限（节点/客户端约 24 条），避免 JSON 无限膨胀。

### 8.3 接口

- `GET /<UUID>/api/stats`：读取聚合结果（与面板相同，需 `ADMIN` 登录 cookie；未设管理密码则与面板一样免登录）。
- `POST /<UUID>/api/stats?op=reset`：仅清空统计。
